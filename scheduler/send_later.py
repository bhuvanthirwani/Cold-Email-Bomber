...
def schedule_emails(scenario):
    ...
    try:
        ...
        if 0 <= hour < 24 and 0 <= minute < 60:
            send_emails_at_specific_time(hour, minute, scenario)
        else:
...
def send_emails_at_specific_time(hour, minute, scenario, batch_size=10):
    """
                ...
                try:
                    ...
                    if isinstance(recipient_emails, tuple):
                        for recipient_email in recipient_emails:
                            subject = f"[Bhuvan Thirwani]: Exploring {designation} Roles at {company_name}"
                            ...
                            send_email(sender_email, sender_password, recipient_email, subject, message, company_name, scenario)
                        ...
                        message = email_template.format(first_name=first_name, last_name=last_name, email=recipient_emails,
                                                         company_name=company_name, designation=designation if designation else "esteemed employee")
                        send_email(sender_email, sender_password, recipient_emails, subject, message, company_name, scenario)
